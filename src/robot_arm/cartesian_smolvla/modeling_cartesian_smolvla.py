...
from robot_arm.robot_schema import CARTESIAN_ACTION_NAMES, PRIMITIVE_COMPLETION
...
class CartesianSmolVLAPolicy(SmolVLAPolicy):
    ...
    def __init__(self, config: CartesianSmolVLAConfig, **kwargs):
        ...
        assert config.max_action_dim == config.action_feature.shape[0] == len(CARTESIAN_ACTION_NAMES), (
            "Cartesian VLA requires exactly seven action dimensions in the dataset and model"
        )
