...
class RobotEnv(gym.Env):
    ...
    def __init__(self, arm: Arm, max_seconds: float, height: int, width: int, trajectory_length: int, trajectory_dim: int, pose_distance_weights: np.ndarray):
        super().__init__()
        self.arm = arm
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "low_level_trajectory_goal": spaces.Box(  
                    ...
                    low=-1.0, high=1.0, shape=(trajectory_length, trajectory_dim), dtype=np.float32
                ),
            }
        )
        # We store the current goals so they can be accessed in step() and _get_obs()
        self.current_trajectory_goal = np.zeros((trajectory_length, trajectory_dim), dtype=np.float32)
    ...
    def reset(self, seed=None, options=None) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        if options is None or "low_level_trajectory_goal" not in options or "original_agent_pos" not in options:
            raise ValueError("Must provide 'low_level_trajectory_goal' and 'original_agent_pos' in reset options.")
            
        self.current_trajectory_goal = np.array(options["low_level_trajectory_goal"], dtype=np.float32)
        self.original_agent_pos = np.array(options["original_agent_pos"], dtype=np.float32)
    ...
    def _get_closest_path_point(self, current_point: np.ndarray, path: np.ndarray) -> Tuple[np.ndarray, int, float]:
        """
        ...
        """
        if path.shape[0] < 2:
            raise ValueError("Path must contain at least 2 waypoints to define progressing segments.")
    ...
    def _compute_path_progress(self, path: np.ndarray, segment_idx: int, t: float) -> float:
        """
    ...
    def compute_reward(self) -> float:
        try:
            ...
            closest_pt, seg_idx, t = self._get_closest_path_point(current_pinch, trajectory)
