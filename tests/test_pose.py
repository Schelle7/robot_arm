...
def test_delta_to_is_inverse_of_apply_delta():
    start = Pose.from_euler([0.2, -0.1, 0.3], [0.4, -0.2, 0.1], 0.25, "XYZ", False)
    target = Pose.from_euler([-0.3, 0.5, 0.1], [-0.1, 0.3, -0.4], 0.75, "XYZ", False)

    reconstructed = start.apply_delta(start.delta_to(target))

    np.testing.assert_allclose(reconstructed.position, target.position, atol=1e-6)
    np.testing.assert_allclose(reconstructed.as_matrix(), target.as_matrix(), atol=1e-6)
    np.testing.assert_allclose(reconstructed.gripper, target.gripper, atol=1e-6)
