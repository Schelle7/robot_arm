...
from diagnostics import inspect_hardware
...
def test_grouped_read_failure_preserves_individual_results(monkeypatch):
    ...
    monkeypatch.setattr(inspect_hardware, "read_block", failed_block)
    result = inspect_hardware.inspect_bus(ReadOnlyBus())
...
def test_unresponsive_motors_are_reported_without_false_calibration_mismatches(monkeypatch, missing):
    monkeypatch.setattr(inspect_hardware, "read_block", lambda bus: {"sample": "ok"})
    result = inspect_hardware.inspect_bus(ReadOnlyBus(missing))
...
def test_compare_modes_measures_feedback_and_restores_original_mode(monkeypatch, original_mode):
    monkeypatch.setattr(inspect_hardware.time, "sleep", lambda seconds: None)
    bus = ModeBus(mode=original_mode)
    result = inspect_hardware.compare_position_modes(bus)["pan"]
...
def test_compare_modes_refuses_enabled_torque():
    ...
    with pytest.raises(RuntimeError, match="torque already OFF"):
        inspect_hardware.compare_position_modes(bus)
    assert bus.writes == []
